...
# ------------------------------------------------------------
# OpenLORIS runner (images only) — supports chunked and unchunked inputs
#
# Unchunked per-sequence layout (from undistort_openloris_sequences.py):
#   ${seq}/images/                 (all frames)
#   ${seq}/timestamp.csv           ("timestamp filename", seconds)
#   ${seq}/groundtruth.txt         (TUM format: "timestamp tx ty tz qx qy qz qw")
#
# Chunked per-sequence layout:
#   ${seq}/images/part_0001/, part_0002/, ... (each ~N frames)
#   ${seq}/timestamp_0001.csv, timestamp_0002.csv, ...   (seconds)
#   ${seq}/groundtruth.txt
#
...
#   - Always copies groundtruth.txt (already TUM) to outputs
#   - Mirrors results to ./output_vipe[_EXPNAME]/poses_and_groundtruth/<seq>/
#   - Optionally runs your evo benchmark (benchmark_dx.py)
#
# Usage:
#   bash processing_scripts/benchmark_vipe_OpenLORIS.sh [-e EXPNAME] [-p PIPELINE] [-v]
#                                     [--input-root PATH]
...
# ------------------------------------------------------------
# ./processing_scripts/benchmark_vipe_OpenLORIS.sh -e OpenLORIS_gt_intr -p TUM-VI --input-root ../data/OpenLORIS_dx/ --scale-all --bench-py ../rss-ros-envs/scripts/evo_benchmark/benchmark_dx.py 
# ./processing_scripts/benchmark_vipe_OpenLORIS.sh -e OpenLORIS_gt_intr_dav3 -p TUM-VI --input-root ../data/OpenLORIS_dx/ --scale-all --bench-py ../rss-ros-envs/scripts/evo_benchmark/benchmark_dx.py
# ./processing_scripts/benchmark_vipe_OpenLORIS.sh -e OpenLORIS_gt_intr_dav3 -p TUM-VI --input-root /home/ubuntu/data/ADI_dataset_extracted --scale-all --bench-py ../rss-ros-envs/scripts/evo_benchmark/benchmark_dx.py


# Defaults (OpenLORIS)
ROOT_IN="/home/dxue/workspaces/isaac_ros-dev/OpenLORIS_dx"
EXPNAME=""
PIPELINE="default"
VISUALIZE=0
ONLY_LIST=()
...
# Convert ViPE pose npz + timestamp.csv (seconds OR ns) -> TUM txt (seconds)
...
def load_timestamps_from_csv(path):
    ...
    Accepts lines like:
      - "1560000003.537705, camera/....png"      (comma)
      - "1560000003.537705 camera/....png"       (space / tabs)
      - "# comment..."                           (ignored)
      - "timestamp,filename" or "timestamp filename" headers (ignored)

    First token is parsed as seconds if it has a '.', otherwise as integer;
    large integers (>1e12) are treated as nanoseconds and divided by 1e9.
    """
    ts_sec = []
    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            ...
            if line.startswith('#'):
                continue
            # drop inline comments like "..., # note"
            line = line.split('#', 1)[0].strip()
            if not line:
                continue

            # split: prefer comma if present, else split on whitespace
            if ',' in line:
            ...
            if not re.fullmatch(r'[+-]?\d+(\.\d+)?', tok):
                # tolerate stray BOMs or weird tokens by skipping
                continue

            try:
                if '.' in tok:
                    ts = float(tok)  # seconds as float
                else:
                    val = int(tok)
                    # Heuristic: treat very large ints as nanoseconds
...
def main():
    ...
    ap.add_argument('--timestamp-csv', required=True, help='timestamp.csv (seconds or ns; comma or whitespace separated)')
    ...
    ts = load_timestamps_from_csv(args.timestamp_csv)
    if len(ts) == 0:
        print(f"[ERR] No usable timestamps found in {args.timestamp_csv}", file=sys.stderr); sys.exit(2)
...
echo "[INFO] Scale-all APE: $([[ ${SCALE_ALL} -eq 1 ]] && echo ON || echo OFF)"
...
for seq_dir in "${ROOT_IN}"/*; do
  ...
  [[ -f "${gt_txt}" ]] || { echo "[SKIP] ${seq_name}: missing groundtruth.txt"; have_inputs=0; }
    ...
    if is_chunked_seq "${seq_dir}"; then
      ...
      for cid in "${chunk_ids[@]}"; do
        ...
        if [[ -z "${npz_path}" && ${SKIP_INFER} -ne 1 ]]; then
          ...
          echo "[ENV ] PYTORCH_CUDA_ALLOC_CONF=${PYTORCH_CUDA_ALLOC_CONF}" | tee -a "${part_log}"
      ...
      # Groundtruth (copy as-is)
      gt_tum="${out_seq_dir}/groundtruth.txt"
      echo "[GT] Copy ${gt_txt} -> ${gt_tum}" | tee -a "${log}"
      cp -f "${gt_txt}" "${gt_tum}"
